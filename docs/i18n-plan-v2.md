# i18n Plan v2 — Slovak (SK) Language Support

Source: i18n-plan.md (v1) + Fable 5 review (`docs/i18n-plan-review.md`)
Status: all 14 Fable points adopted + 2 user forks resolved.

This plan is the source of truth. v1 is superseded.
...
`Astro.currentLocale` resolves correctly on every prerendered route in Astro 7, including the asymmetric root-vs-`/sk/` case. **No manual fallback** (C1) — use `getContent(Astro.currentLocale)` directly.

### 1.2 Content structure — per-locale bundle files

**CRITICAL (A2):** The current content files export multiple separate named consts (`site.ts` exports `site`, `primaryCta`, `secondaryCta`, `tertiaryCta`, `trustLine`). This must be restructured into a single bundle object per locale BEFORE `getContent()` or `check-translations.ts` can be written.
...
Components call `getContent(Astro.currentLocale)` — no fallback (C1).
...
- `Header.astro` — nav labels, CTAs, ecosystem links, language switch (mobile: inside menu panel, B2)
...
- `AuditRequestForm.tsx` — all UI strings, field labels, placeholders, button text, success/error states, privacy notice, Turnstile `language: locale` (B3)

---

## 4. Form i18n

### 4.1 Enum refactor (no backwards-compat hedge — A6)
...
### 4.2 Webhook payload (IDs only, no dual-field hedge)
...
**Tradeoff (B4, stated explicitly):** both locales' strings ship in the client bundle regardless of which page loaded. At 2 locales this is a few KB — acceptable. Revisit if more locales are added.

### 4.5 Turnstile language (B3)
...
### 5.3 Sitemap (A5 fix: en-GB, not en-EU)
...
- **Mobile (B2):** inside the mobile menu panel, not the sticky header row (which has only room for ThemeToggle + hamburger)
- Also in Footer
...
**D4:** Confirm Umami Cloud plan tier supports custom properties. If a "dimension" requires an upgrade, event properties (above) are sufficient and already included.
...
- **D3:** SK legal pages get their own `lastUpdated` (independent of EN — legal review timing won't match)
...
## 10. CI guard (created from scratch — A4)

**A4 finding:** the "existing CI grep guard from plan v3 Phase 9" was planned but never shipped. Create it now for both locales.
...
### 10.2 Translation completeness (SK files only — B1)

```ts
// scripts/check-translations.ts
// Written AFTER content restructure (A2) lands, against the actual bundle shape.
// Only runs when src/content/sk/** files change.
// Fails if any SK key is missing or empty.
```
...
## 11. Translation completeness enforcement (B1 scope)
...
- **Restructure content files into per-locale bundle objects** (A2):
...
- Create `lib/i18n.ts` (`getContent`, `getAlternatePath` — no `getLocaleFromUrl` fallback, C1)
- Create `lib/translations.ts` (form UI strings, both locales)
- Refactor `validation.ts` enum values to stable IDs + localized labels (no dual-field hedge, A6)
...
- Also: ship EN-only grep guard now (overdue from plan v3 Phase 9)
...
- [ ] hreflang validated (Google Search Console) — will pass now that `en_GB` is valid (A5)
- [ ] Sitemap includes both locales with alternates
- [ ] Lighthouse 95+ on `/sk/` (mobile + desktop)
- [ ] No untranslated EN strings on SK pages
- [ ] SK OG image exists (`/sk/og.png`) — required before launch (D2)
- [ ] Section anchor IDs are locale-invariant (D1)
...
.github/ISSUE_TEMPLATE/legal-review.md
public/sk/og.png
```
...
src/content/*.ts → en/*.ts    # moved + restructured into bundle objects (A2)
src/lib/validation.ts         # enum IDs + localized labels (no hedge, A6)
...
.github/workflows/ci.yml      # add content guard + translation check + SK smoke test
```
...
- Does not add backwards-compat for webhook enum values (no consumer exists, A6)
